...
Live tests produced H1s like `"Kinderbrillen &amp; Jugendbrillen 2026: Günstige Komplettbrillen richtig auswählen"` — buying-guide phrasing publishers reject. Three coupled causes: brainstorm sorted by SEO volume so commercial keywords took rank 0 (the auto-pick); contract's "h1 must contain target_keyword" re-injected the SEO keyword over any editorial angle; Sonnet pre-escaped `&` → `&amp;` in JSON titles.

Target shape: commercial intent stays in `target_keyword`, but the *title* wraps it in a publisher-acceptable frame — e.g. `"Kinderbrille auf Rezept: Wie viel zahlt die Krankenkasse — und wo lohnt sich der Online-Kauf?"`.

- **Brainstorm DE+FR** (`topic_brainstorm.py`): rank by editorial publishability, SEO volume = tiebreaker. Six required title frames (question / study / regulatory / explainer / reportage / fact-check). Hard blocklist of commercial qualifiers in *title* only (`günstig`, `kaufen`, `Vergleich`, `Test`, `Beste`, etc., + FR equivalents) — keyword field unchanged.
- **Contract DE+FR** (`contract_generator/v1.{de,fr}.md`): when angle present, `h1 = angle.title` verbatim and keyword-in-h1 rule suspended (SEO moves to `meta_title`). H1 cap 12–96 → 12–160. Same blocklist applies. Year no longer forced into H1.
- **HTML-entity heal**: `html.unescape()` on parse in `_parse_angles` + `_hydrate_angles_from_cache`; mirror helper in `contract_generator.py` over h1/meta/H2/FAQ before Pydantic validation.
- **Cache bust**: `BRAINSTORM_CACHE_VERSION v1 → v2` invalidates 90-day batches generated under the old prompt.

Tests: 320 creator + 139 portal, all green (suite asserts wiring, not phrasing).

## Phase E — Publisher selector rebuild · ✅ DONE (2026-05-06, `ecbca1c` + `a5aa573`)

Live `brillenhaus24.de` test (no publishing site) hard-failed: Phase D's fit gate ran Haiku on the deterministic top-1 only (a Klimaschutz site) and gave up. Pre-contract fit-check + post-contract reselection were also out of sync — contract was generated before publisher was known.

Replaced the chain with one Haiku call that reranks the full K=8 shortlist *relatively* and picks winner + refined topic. Publisher locked in BEFORE contract.

- **`creator/api/publisher_selector.py`** (new) — `select_best_publisher(...)` → `SelectionResult{best_site_id, refined_topic, confidence, no_fit, ranking, soft_passed}`. Single Haiku call. DE+FR prompts. Soft-passes to deterministic top on infra failure.
- **`POST /v2/select-publisher`** — `no_fit=true` is a verdict, not an error.
- **Portal `_run_create_article_pipeline_v2`** — calls selector pre-contract. On `no_fit` falls back to `is_general=True` candidate (else AutomationError). On selector-down falls back to deterministic top. `/v2/run-pipeline` invoked with `publishing_site_url=<chosen>` (no more late-bind).
- **Deleted** (~485 lines): `publisher_fit.py`, `/v2/refine-topic-for-publisher`, `select_publish_target_after_contract`, `pre_flight_publishing_feasibility`, `_topical_fit_score`, `_select_publish_target_for_v2`, `_candidate_language`, `_similarity_score`, `_flatten_candidate_text`, `_LATE_BIND_FIT_FLOOR`, `test_phase_c_late_binding.py`.
- **`_derive_keyword_from_target_profile` skips `page_title`** — thin profiles fall through to `/v2/derive-topic` (DataForSEO-clean keyword) instead of feeding raw HTML titles into the selector.

### Phase E follow-up (`a5aa573`) — title-shaped keywords blocked

Second live test failed at research with DataForSEO 40501 (`Augengesundheit und Sehhilfen: Wie die richtige Brille zu deinem Lifestyle passt` — 12 words, colon). LLM drift put a title in a keyword field. Three layers:

1. **`research._sanitize_seo_keyword`** — entry-point gate; splits at `:` `—` `|` `?` `!`, caps at 8 words. DataForSEO never sees a multi-clause string.
2. **Portal `_looks_like_seo_keyword`** — gates both brainstorm `top_keyword` and selector `refined_topic` overrides; rejected values trace-log and keep the cleaner upfront keyword.
3. **Selector prompt** — `refined_topic` redefined as 1–4 word SEO keyword with positive + negative examples (DE + FR).
...
### Phase E follow-up #5 — overload-class retry budget across all LLM calls

Live test still 529'd, this time on `voice_pass`. Root cause: `call_llm_text` had `retries=0` *hardcoded* — every voice_pass call had a single attempt, no retry. `call_llm_json` defaulted to `retries=1` (2 attempts), too tight for actual overload events. Fixes:
- Parameterize `call_llm_text` retries (was hardcoded).
- Both `call_llm_json` and `call_llm_text` now default to `retries=4` / `backoff=2.0s` → 5 attempts at 0/2/4/8/16s ≈ 30s budget, enough for typical overload events without making a clean run hang.
- Add multiplicative jitter (0.8-1.2×) via shared `_retry_sleep_seconds()` so concurrent retries spread instead of dogpiling Anthropic during recovery. Reused in `contract_generator.call_with_thinking`.
- `section_writer.DEFAULT_RETRIES` 2→4 to match (sections is a critical phase, voice now has 5 attempts; sections shouldn't be weaker).
- `call_with_thinking` `max_attempts` 3→5 for parity.
...
- ✅ **Phase D live validation** — confirmed via Phase E debug runs; angles like `"Augengesundheit und Sehhilfen: Wie die richtige Brille…"` are well-formed editorial titles. Regression lever if needed: split brainstorm into `editorial_top`/`seo_long_tail` lists, auto-pick from `editorial_top[0]`.
- 🔜 **Admin selector preview** — surface `ranking[0..2]` + rationale in job-detail UI for override before content gen. Read-only on existing data.
- 🔜 **Publisher-aware contract** — feed chosen publisher profile (audience, primary context, sample headlines) into `contract_generator.py`. Quality win at zero infra cost.
