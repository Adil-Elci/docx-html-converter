...
- `CREATOR_PHASE2_MAX_TOKENS` (default: 3000)
- `CREATOR_STRICT_FAILURE_MODE` (default: false; when true, Creator disables retries and fallbacks so the first failing phase raises immediately)
- `CREATOR_PHASE4_MAX_TOKENS` (default: 3000)
- `CREATOR_PHASE5_MAX_ATTEMPTS` (default: 2; forced to 1 when `CREATOR_STRICT_FAILURE_MODE=true`)
- `CREATOR_PHASE5_MAX_TOKENS_ATTEMPT1` (default: 3000)
- `CREATOR_PHASE5_MAX_TOKENS_RETRY` (default: 3000; only used when retries are enabled)
- `CREATOR_PHASE7_REPAIR_ATTEMPTS` (default: 1; forced to 0 when `CREATOR_STRICT_FAILURE_MODE=true`)
