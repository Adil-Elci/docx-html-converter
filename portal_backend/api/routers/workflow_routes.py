...
from ..workflow_schemas import WorkflowBoardOut, WorkflowCardMoveIn, WorkflowCardOut, WorkflowColumnOut
...
def _ensure_default_workflow_columns(db: Session) -> List[WorkflowColumn]:
    ...
    for spec in DEFAULT_WORKFLOW_COLUMNS:
        ...
        if column is None:
            ...
            changed = True
            continue
        if column.name != spec["name"] or column.color != spec["color"] or column.position != spec["position"]:
